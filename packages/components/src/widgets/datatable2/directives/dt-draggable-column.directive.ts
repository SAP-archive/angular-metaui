...
export class DTDraggableColumnDirective implements OnInit, OnDestroy
{
    ...
    {
        let isDragHandleClicked = event.target.classList.contains('icon-grab');
    ...
    {
        let wrapper = this.domUtils.closest(event.target, '.dt-body-wrapper');
    ...
    private onDragOverEvent(event: any): void
    {
        ...
        let currCol = this.domUtils.closest(event.target, 'th') || event.target;
        let currColCenter = this.domUtils.elementDimensions(currCol).width / 2;
    ...
    private onDropEvent(event: any): void
    {
        ...
        let dropPos: DropPosition = this.dropPos === DragColumnDirection.Left ?
    ...
    {
        let td = this.domUtils.closest(event.target, 'th') || event.target;
    ...
    private isValidDropTarget(event: any): boolean
    {
        ...
        let siblingCol = this.dndIndex - origInx;

        // We want to limit dropping into the next subling col
        let isRighHalf = this.dragDir === DragColumnDirection.Right &&
            this.dropPos === DragColumnDirection.Right;
        let isLeftHalf = this.dragDir === DragColumnDirection.Left &&
            this.dropPos === DragColumnDirection.Left;
    ...
    {
        let source = this.dt.env.getValue('dragSource');
        let target = this.tablePos(event);
